...
pub fn start_system_audio_capture(out_tx: Sender<PcmChunk>) -> Result<SystemAudioHandle, String> {
    let stop = Arc::new(AtomicBool::new(false));
    let stop_flag = stop.clone();
    let thread = thread::Builder::new()
        .name("system-audio".into())
        .spawn(move || {
            if let Err(e) = system_loop(out_tx, stop_flag) {
                logging::error(format!("System audio capture failed: {e}"));
            }
        })
...
fn system_loop(out_tx: Sender<PcmChunk>, stop: Arc<AtomicBool>) -> Result<(), String> {
    ...
    logging::info("System audio loopback capture started");
